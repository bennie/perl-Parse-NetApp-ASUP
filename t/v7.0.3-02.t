...
md5_hex($extract_aggr_status) eq 'c5d97a2264357f30342ce7bd05896f21' ? ok(7) : nok(7);
...
length($extract_cluster_monitor) eq '2741' ? ok(24) : nok(24);
md5_hex($extract_cluster_monitor) eq 'f6f5940a1015653302cffd0b7fb8ab94' ? ok(25) : nok(25);
substr($extract_cluster_monitor,0,20) eq '===== CLUSTER MONITO' ? ok(26) : nok(26);

my $extract_df = $pna->extract_df();
length($extract_df) eq '1052' ? ok(27) : nok(27);
md5_hex($extract_df) eq '584329554bd34d520615b89551f8a89c' ? ok(28) : nok(28);
substr($extract_df,0,20) eq '===== DF =====
Files' ? ok(29) : nok(29);

my $extract_df_a = $pna->extract_df_a();
length($extract_df_a) eq '210' ? ok(30) : nok(30);
md5_hex($extract_df_a) eq '35eaf2033a5047ba52f52f371ced2df7' ? ok(31) : nok(31);
substr($extract_df_a,0,20) eq '===== DF-A =====
Agg' ? ok(32) : nok(32);

my $extract_df_i = $pna->extract_df_i();
length($extract_df_i) eq '482' ? ok(33) : nok(33);
md5_hex($extract_df_i) eq '10bc94829cc8007cf11c3068ce4f4bd4' ? ok(34) : nok(34);
substr($extract_df_i,0,20) eq '===== DF-I =====
Fil' ? ok(35) : nok(35);

my $extract_df_r = $pna->extract_df_r();
length($extract_df_r) eq '1076' ? ok(36) : nok(36);
md5_hex($extract_df_r) eq '10938aeccc04cb7693b0baab90f8f4f4' ? ok(37) : nok(37);
...
length($extract_dns_info) eq '605' ? ok(42) : nok(42);
md5_hex($extract_dns_info) eq '8ab47b1a0e9c0aa624641a6674e4ef52' ? ok(43) : nok(43);
...
md5_hex($extract_ecc_memory_scrubber_stats) eq '0e167e4d46a2722864188c0c3d5a242f' ? ok(46) : nok(46);
...
length($extract_environment) eq '4201' ? ok(48) : nok(48);
md5_hex($extract_environment) eq '31a59529f222b8334759ac0346c03cf9' ? ok(49) : nok(49);
substr($extract_environment,0,20) eq '===== ENVIRONMENT ==' ? ok(50) : nok(50);

my $extract_exports = $pna->extract_exports();
length($extract_exports) eq '640' ? ok(51) : nok(51);
md5_hex($extract_exports) eq '52284b430fe3212e92fbf12875b3fc97' ? ok(52) : nok(52);
...
md5_hex($extract_failed_disk_registry) eq '14c89cca29d20c5f799c4ac6ae4bd1e0' ? ok(55) : nok(55);
...
length($extract_fc_device_map) eq '653' ? ok(57) : nok(57);
md5_hex($extract_fc_device_map) eq '9fe1aaea5a0c6a786685435c3091e3d9' ? ok(58) : nok(58);
substr($extract_fc_device_map,0,20) eq '===== FC DEVICE MAP ' ? ok(59) : nok(59);

my $extract_fc_link_stats = $pna->extract_fc_link_stats();
length($extract_fc_link_stats) eq '3103' ? ok(60) : nok(60);
md5_hex($extract_fc_link_stats) eq '5c622fe973a036b83b878f5622a43154' ? ok(61) : nok(61);
substr($extract_fc_link_stats,0,20) eq '===== FC LINK STATS ' ? ok(62) : nok(62);

my $extract_fc_stats = $pna->extract_fc_stats();
length($extract_fc_stats) eq '21554' ? ok(63) : nok(63);
md5_hex($extract_fc_stats) eq 'dc352f7dc437b85b116560e954186f81' ? ok(64) : nok(64);
...
md5_hex($extract_flash_card_info) eq 'c54304331210538714cc28002118ffa0' ? ok(85) : nok(85);
...
md5_hex($extract_headers) eq '2b983c4a5ad1f26f7f06f379ea244db6' ? ok(94) : nok(94);
substr($extract_headers,0,20) eq 'GENERATED_ON=Sun Mar' ? ok(95) : nok(95);

my $extract_hosts = $pna->extract_hosts();
length($extract_hosts) eq '445' ? ok(96) : nok(96);
md5_hex($extract_hosts) eq '8a89edaac3838ed696080010b325de05' ? ok(97) : nok(97);
substr($extract_hosts,0,20) eq '===== HOSTS =====
#A' ? ok(98) : nok(98);

my $extract_httpstat = $pna->extract_httpstat();
length($extract_httpstat) eq '155' ? ok(99) : nok(99);
md5_hex($extract_httpstat) eq '6e37e533eb75639130d1ef0c5d5a20d3' ? ok(100) : nok(100);
...
length($extract_ifconfig_a) eq '1230' ? ok(105) : nok(105);
md5_hex($extract_ifconfig_a) eq 'b899e1dec9150d697b9cff20be959825' ? ok(106) : nok(106);
...
length($extract_ifstat_a) eq '8770' ? ok(111) : nok(111);
md5_hex($extract_ifstat_a) eq '95c2c3e9a10a685ce55238541aab0cd6' ? ok(112) : nok(112);
...
length($extract_interconnect_stats) eq '817' ? ok(120) : nok(120);
md5_hex($extract_interconnect_stats) eq '30dfec84aee42ef1af453b6a0904e5a9' ? ok(121) : nok(121);
...
length($extract_messages) eq '35215' ? ok(174) : nok(174);
md5_hex($extract_messages) eq '30cb519cc37664445d5e50d73f3a977a' ? ok(175) : nok(175);
...
md5_hex($extract_netstat_s) eq '8e2d1da3d40acc0d3620f457bb74dadd' ? ok(181) : nok(181);
...
length($extract_nfsstat_d) eq '5903' ? ok(186) : nok(186);
md5_hex($extract_nfsstat_d) eq '929548e8ed6240d4b7b844c4d699b59e' ? ok(187) : nok(187);
...
md5_hex($extract_nis_info) eq 'aed4237c2b2d028c65aae7a7ff35aa86' ? ok(190) : nok(190);
...
md5_hex($extract_nsswitch_conf) eq '6a0ed637f1a38dffcddc2bee4da3c2cb' ? ok(193) : nok(193);
substr($extract_nsswitch_conf,0,20) eq '===== NSSWITCH-CONF ' ? ok(194) : nok(194);

my $extract_options = $pna->extract_options();
length($extract_options) eq '16853' ? ok(195) : nok(195);
md5_hex($extract_options) eq 'a1c6e2339fbca65a32e0b5a7bcf6515c' ? ok(196) : nok(196);
...
length($extract_qtree_status) eq '467' ? ok(204) : nok(204);
md5_hex($extract_qtree_status) eq '2ff2c6cc44953cc29aacc35438b81178' ? ok(205) : nok(205);
...
length($extract_rc) eq '435' ? ok(210) : nok(210);
md5_hex($extract_rc) eq 'c76d8572a5b7326d77ab71c74957cbe8' ? ok(211) : nok(211);
...
md5_hex($extract_resolv_conf) eq '7de9035351b6f150194d584601074cc5' ? ok(214) : nok(214);
...
md5_hex($extract_service_usage) eq '2c33934a1e9afac1919c85c6828b5853' ? ok(235) : nok(235);
...
length($extract_snap_list_n) eq '804' ? ok(261) : nok(261);
md5_hex($extract_snap_list_n) eq 'f57d9e24b01e6c577b5ff538792efe43' ? ok(262) : nok(262);
...
md5_hex($extract_snap_reserve) eq 'ef639134010ae2dc5de21e6940011244' ? ok(268) : nok(268);
...
md5_hex($extract_snap_sched) eq '1ebb6b175a8e917536d33641799b1015' ? ok(274) : nok(274);
...
length($extract_snapmirror_status) eq '49' ? ok(288) : nok(288);
md5_hex($extract_snapmirror_status) eq 'cd404b44add961512816b343e862d8bf' ? ok(289) : nok(289);
...
length($extract_storage) eq '23972' ? ok(312) : nok(312);
md5_hex($extract_storage) eq '54e0e186b2cf39add8bcb2d0dabc2f6e' ? ok(313) : nok(313);
substr($extract_storage,0,20) eq '===== STORAGE =====
' ? ok(314) : nok(314);

my $extract_sysconfig_a = $pna->extract_sysconfig_a();
length($extract_sysconfig_a) eq '8582' ? ok(315) : nok(315);
md5_hex($extract_sysconfig_a) eq '49f2902c850fab89ae97e4d43a8eb54e' ? ok(316) : nok(316);
...
md5_hex($extract_sysconfig_d) eq '182fec3a0d2d73afcd57487be50e15d8' ? ok(325) : nok(325);
...
md5_hex($extract_sysconfig_r) eq 'f87fb393651bf5fd9fe6138b61297e5a' ? ok(334) : nok(334);
...
md5_hex($extract_system_serial_number) eq 'd16bee9ea82f0ce2c52b81a233d77134' ? ok(337) : nok(337);
...
length($extract_usage) eq '1856' ? ok(342) : nok(342);
md5_hex($extract_usage) eq 'c24a03f78e4eb3c9a72f010a1942f9c2' ? ok(343) : nok(343);
...
length($extract_vif_status) eq '886' ? ok(354) : nok(354);
md5_hex($extract_vif_status) eq '98148a8fd5ca820f39cc7268c3f08884' ? ok(355) : nok(355);
...
md5_hex($extract_vol_language) eq '6605d1eb6cd5a6a30e414f7541f5efac' ? ok(361) : nok(361);
substr($extract_vol_language,0,20) eq '===== VOL-LANGUAGE =' ? ok(362) : nok(362);

my $extract_vol_status = $pna->extract_vol_status();
length($extract_vol_status) eq '6206' ? ok(363) : nok(363);
md5_hex($extract_vol_status) eq '9d6c23dfac7bfaaaa3fd2092a02cbd45' ? ok(364) : nok(364);
...
md5_hex($extract_xheader) eq '8a398003a213dda935258b1c86620e56' ? ok(376) : nok(376);

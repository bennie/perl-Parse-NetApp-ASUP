...
length($extract_aggr_status) eq '913' ? ok(6) : nok(6);
md5_hex($extract_aggr_status) eq '78c786c2cf197578384b9623401c08c0' ? ok(7) : nok(7);
...
length($extract_cluster_monitor) eq '2725' ? ok(24) : nok(24);
md5_hex($extract_cluster_monitor) eq '09c9d400af35509ddf8a9d3607c95658' ? ok(25) : nok(25);
substr($extract_cluster_monitor,0,20) eq '===== CLUSTER MONITO' ? ok(26) : nok(26);

my $extract_df = $pna->extract_df();
length($extract_df) eq '1011' ? ok(27) : nok(27);
md5_hex($extract_df) eq 'ae46341a3e29a395fa42be252b530f8d' ? ok(28) : nok(28);
substr($extract_df,0,20) eq '===== DF =====
Files' ? ok(29) : nok(29);

my $extract_df_a = $pna->extract_df_a();
length($extract_df_a) eq '210' ? ok(30) : nok(30);
md5_hex($extract_df_a) eq 'f252d151e4c833c108b382f2233deaa2' ? ok(31) : nok(31);
substr($extract_df_a,0,20) eq '===== DF-A =====
Agg' ? ok(32) : nok(32);

my $extract_df_i = $pna->extract_df_i();
length($extract_df_i) eq '442' ? ok(33) : nok(33);
md5_hex($extract_df_i) eq '8d2e1f9cc1c542a4e8baef211606f155' ? ok(34) : nok(34);
substr($extract_df_i,0,20) eq '===== DF-I =====
Fil' ? ok(35) : nok(35);

my $extract_df_r = $pna->extract_df_r();
length($extract_df_r) eq '1035' ? ok(36) : nok(36);
md5_hex($extract_df_r) eq '12557f10bd2046f889fc45395961daa7' ? ok(37) : nok(37);
...
length($extract_dns_info) eq '531' ? ok(42) : nok(42);
md5_hex($extract_dns_info) eq 'b3c0cca98505a65866d241f6170bf145' ? ok(43) : nok(43);
...
md5_hex($extract_ecc_memory_scrubber_stats) eq '9935aa401adcb847e6b24a0b4d46a1fa' ? ok(46) : nok(46);
...
md5_hex($extract_environment) eq 'b77017864b6169ff7d7507c3455bafef' ? ok(49) : nok(49);
substr($extract_environment,0,20) eq '===== ENVIRONMENT ==' ? ok(50) : nok(50);

my $extract_exports = $pna->extract_exports();
length($extract_exports) eq '293' ? ok(51) : nok(51);
md5_hex($extract_exports) eq '32b23988a8885e5c69db26b78a5dae07' ? ok(52) : nok(52);
substr($extract_exports,0,20) eq '/vol/vol_grin_cspi_d' ? ok(53) : nok(53);

my $extract_failed_disk_registry = $pna->extract_failed_disk_registry();
length($extract_failed_disk_registry) eq '246' ? ok(54) : nok(54);
md5_hex($extract_failed_disk_registry) eq '60f66a12904a05c5c68da3a1afc80625' ? ok(55) : nok(55);
...
md5_hex($extract_fc_device_map) eq '523ed912166dbee19d4ca1f466387b1d' ? ok(58) : nok(58);
substr($extract_fc_device_map,0,20) eq '===== FC DEVICE MAP ' ? ok(59) : nok(59);

my $extract_fc_link_stats = $pna->extract_fc_link_stats();
length($extract_fc_link_stats) eq '5303' ? ok(60) : nok(60);
md5_hex($extract_fc_link_stats) eq '6ed2840e1831b0969717326d44e53e82' ? ok(61) : nok(61);
substr($extract_fc_link_stats,0,20) eq '===== FC LINK STATS ' ? ok(62) : nok(62);

my $extract_fc_stats = $pna->extract_fc_stats();
length($extract_fc_stats) eq '25880' ? ok(63) : nok(63);
md5_hex($extract_fc_stats) eq '3143f6d363e4dc7d9687eac4e757d762' ? ok(64) : nok(64);
...
md5_hex($extract_flash_card_info) eq '256c68fdcbea667e8647fe92694b93f8' ? ok(85) : nok(85);
...
length($extract_headers) eq '301' ? ok(93) : nok(93);
md5_hex($extract_headers) eq '72b3387e3e82a930f3281c742bc3fdcb' ? ok(94) : nok(94);
...
md5_hex($extract_httpstat) eq 'be96ec9e4efc1f29ce704ee17d149be5' ? ok(100) : nok(100);
...
md5_hex($extract_ifconfig_a) eq '2398171c1eb942ad9ccdf2d0ac37b92e' ? ok(106) : nok(106);
...
md5_hex($extract_ifstat_a) eq '8268b388667320f2aa81d5d6618e07cd' ? ok(112) : nok(112);
...
length($extract_interconnect_stats) eq '511' ? ok(120) : nok(120);
md5_hex($extract_interconnect_stats) eq '055bea7f9125c4806ec695ec003eac7d' ? ok(121) : nok(121);
...
length($extract_lun_configuration) eq '208' ? ok(165) : nok(165);
md5_hex($extract_lun_configuration) eq '16525ead8e4c7f261b982b9f59903562' ? ok(166) : nok(166);
...
length($extract_messages) eq '2168812' ? ok(174) : nok(174);
md5_hex($extract_messages) eq '2e86c3f695b85aad72a9fe680aeffbe1' ? ok(175) : nok(175);
...
length($extract_netstat_s) eq '3968' ? ok(180) : nok(180);
md5_hex($extract_netstat_s) eq '392bafc0fc36a88d5926670f22fa6c11' ? ok(181) : nok(181);
...
length($extract_nfsstat_d) eq '5854' ? ok(186) : nok(186);
md5_hex($extract_nfsstat_d) eq '56ddc559184aa911c8271118bec9ce35' ? ok(187) : nok(187);
...
md5_hex($extract_nis_info) eq 'ccf519e27ceaa9c2b5766633464b61b6' ? ok(190) : nok(190);
...
md5_hex($extract_options) eq '4b18ec9ca5fbec9c61747ff51ef3633b' ? ok(196) : nok(196);
...
length($extract_qtree_status) eq '790' ? ok(204) : nok(204);
md5_hex($extract_qtree_status) eq '42e34517cf013627c10e52a3b0efbb63' ? ok(205) : nok(205);
...
length($extract_service_usage) eq '577' ? ok(234) : nok(234);
md5_hex($extract_service_usage) eq 'd904605cf69e1cd2423b4ff2d8c848b2' ? ok(235) : nok(235);
...
length($extract_snap_list_n) eq '698' ? ok(261) : nok(261);
md5_hex($extract_snap_list_n) eq '857d137b1d69e9b432ae767eb199d853' ? ok(262) : nok(262);
substr($extract_snap_list_n,0,20) eq '===== SNAP-LIST-N ==' ? ok(263) : nok(263);

my $extract_snap_list_n_a = $pna->extract_snap_list_n_a();
length($extract_snap_list_n_a) eq '247' ? ok(264) : nok(264);
md5_hex($extract_snap_list_n_a) eq '31b70f03adf6ac011cd011e1e22584bd' ? ok(265) : nok(265);
substr($extract_snap_list_n_a,0,20) eq '===== SNAP-LIST-N-A ' ? ok(266) : nok(266);

my $extract_snap_reserve = $pna->extract_snap_reserve();
length($extract_snap_reserve) eq '396' ? ok(267) : nok(267);
md5_hex($extract_snap_reserve) eq '8626e68f1692e3f4be0916ed544e6441' ? ok(268) : nok(268);
substr($extract_snap_reserve,0,20) eq '===== SNAP-RESERVE =' ? ok(269) : nok(269);

my $extract_snap_reserve_a = $pna->extract_snap_reserve_a();
length($extract_snap_reserve_a) eq '97' ? ok(270) : nok(270);
md5_hex($extract_snap_reserve_a) eq 'ca89d76985c5c13f0ddfca1852895865' ? ok(271) : nok(271);
substr($extract_snap_reserve_a,0,20) eq '===== SNAP-RESERVE-A' ? ok(272) : nok(272);

my $extract_snap_sched = $pna->extract_snap_sched();
length($extract_snap_sched) eq '216' ? ok(273) : nok(273);
md5_hex($extract_snap_sched) eq '33ee585b3098d5aba96739db77f4d880' ? ok(274) : nok(274);
...
length($extract_storage) eq '45376' ? ok(312) : nok(312);
md5_hex($extract_storage) eq 'e625ea0b49c55e7553bfd6114f78c803' ? ok(313) : nok(313);
substr($extract_storage,0,20) eq '===== STORAGE =====
' ? ok(314) : nok(314);

my $extract_sysconfig_a = $pna->extract_sysconfig_a();
length($extract_sysconfig_a) eq '10646' ? ok(315) : nok(315);
md5_hex($extract_sysconfig_a) eq 'dede708683f523e63fe9be2b7ce34819' ? ok(316) : nok(316);
...
length($extract_sysconfig_d) eq '3253' ? ok(324) : nok(324);
md5_hex($extract_sysconfig_d) eq '911372885b79fa9f20ac901df1ef2849' ? ok(325) : nok(325);
...
length($extract_sysconfig_r) eq '5605' ? ok(333) : nok(333);
md5_hex($extract_sysconfig_r) eq 'd27d5618ba706b09d43940bb942ce1a4' ? ok(334) : nok(334);
...
md5_hex($extract_system_serial_number) eq '37cee3b51cb1be0357b914b1077aedec' ? ok(337) : nok(337);
...
length($extract_usage) eq '2534' ? ok(342) : nok(342);
md5_hex($extract_usage) eq 'e012a8544e0fc32fa4eb7cb1da4117c4' ? ok(343) : nok(343);
...
length($extract_vif_status) eq '1245' ? ok(354) : nok(354);
md5_hex($extract_vif_status) eq '58fcabcda87c7597c1ee05effc2acc0c' ? ok(355) : nok(355);
...
length($extract_vol_language) eq '257' ? ok(360) : nok(360);
md5_hex($extract_vol_language) eq 'ca6a293df89575505dedc1bd8004096a' ? ok(361) : nok(361);
substr($extract_vol_language,0,20) eq '===== VOL-LANGUAGE =' ? ok(362) : nok(362);

my $extract_vol_status = $pna->extract_vol_status();
length($extract_vol_status) eq '6064' ? ok(363) : nok(363);
md5_hex($extract_vol_status) eq 'f5d21d1e01369b31a38a536f34f573a7' ? ok(364) : nok(364);
...
length($extract_xheader) eq '613' ? ok(375) : nok(375);
md5_hex($extract_xheader) eq 'd056d3d5baef7b3f55f0f55eb86025d4' ? ok(376) : nok(376);

...
length($extract_aggr_status) eq '951' ? ok(6) : nok(6);
md5_hex($extract_aggr_status) eq '69f902de5c39bb9ff20f48da9899e098' ? ok(7) : nok(7);
...
length($extract_df) eq '1577' ? ok(27) : nok(27);
md5_hex($extract_df) eq '033b3e709c76601fa67dbd73f7afc293' ? ok(28) : nok(28);
substr($extract_df,0,20) eq '===== DF =====
Files' ? ok(29) : nok(29);

my $extract_df_a = $pna->extract_df_a();
length($extract_df_a) eq '210' ? ok(30) : nok(30);
md5_hex($extract_df_a) eq 'e1d40f2548f1abf49414564123adc1cf' ? ok(31) : nok(31);
substr($extract_df_a,0,20) eq '===== DF-A =====
Agg' ? ok(32) : nok(32);

my $extract_df_i = $pna->extract_df_i();
length($extract_df_i) eq '662' ? ok(33) : nok(33);
md5_hex($extract_df_i) eq '7c255f2104d8a07baff101c28c3e2348' ? ok(34) : nok(34);
substr($extract_df_i,0,20) eq '===== DF-I =====
Fil' ? ok(35) : nok(35);

my $extract_df_r = $pna->extract_df_r();
length($extract_df_r) eq '1617' ? ok(36) : nok(36);
md5_hex($extract_df_r) eq 'c23ffa128bff0ef48e760e29a89d1685' ? ok(37) : nok(37);
...
length($extract_dns_info) eq '646' ? ok(42) : nok(42);
md5_hex($extract_dns_info) eq 'd56ebec4fdc600dc36b96c2d3c6b322d' ? ok(43) : nok(43);
...
md5_hex($extract_ecc_memory_scrubber_stats) eq 'd54d7f535cee8411c383661dd19d7736' ? ok(46) : nok(46);
...
length($extract_environment) eq '2473' ? ok(48) : nok(48);
md5_hex($extract_environment) eq 'b491209d785f19f0e2565ce4c3d444f7' ? ok(49) : nok(49);
substr($extract_environment,0,20) eq '===== ENVIRONMENT ==' ? ok(50) : nok(50);

my $extract_exports = $pna->extract_exports();
length($extract_exports) eq '653' ? ok(51) : nok(51);
md5_hex($extract_exports) eq 'b4b764b6e32705eff3a036744692ada7' ? ok(52) : nok(52);
...
length($extract_fc_device_map) eq '545' ? ok(57) : nok(57);
md5_hex($extract_fc_device_map) eq '9825b819765c92a477affe5c49776d7c' ? ok(58) : nok(58);
substr($extract_fc_device_map,0,20) eq '===== FC DEVICE MAP ' ? ok(59) : nok(59);

my $extract_fc_link_stats = $pna->extract_fc_link_stats();
length($extract_fc_link_stats) eq '3025' ? ok(60) : nok(60);
md5_hex($extract_fc_link_stats) eq '11b2f4a7c5b7251e84cffc112b935e16' ? ok(61) : nok(61);
substr($extract_fc_link_stats,0,20) eq '===== FC LINK STATS ' ? ok(62) : nok(62);

my $extract_fc_stats = $pna->extract_fc_stats();
length($extract_fc_stats) eq '16144' ? ok(63) : nok(63);
md5_hex($extract_fc_stats) eq 'bbf5e521f6cd4c57d71ab7ad732e93c8' ? ok(64) : nok(64);
...
length($extract_flash_card_info) eq '2216' ? ok(84) : nok(84);
md5_hex($extract_flash_card_info) eq 'fb2d9fd690b884af56af8afeb23cc4d6' ? ok(85) : nok(85);
...
md5_hex($extract_headers) eq '4cd6943ccc9080006ba1ad2cf4fea4ff' ? ok(94) : nok(94);
substr($extract_headers,0,20) eq 'GENERATED_ON=Sun Mar' ? ok(95) : nok(95);

my $extract_hosts = $pna->extract_hosts();
length($extract_hosts) eq '329' ? ok(96) : nok(96);
md5_hex($extract_hosts) eq 'c5359c6b190b5de0a249829b7d3acff4' ? ok(97) : nok(97);
substr($extract_hosts,0,20) eq '===== HOSTS =====
#A' ? ok(98) : nok(98);

my $extract_httpstat = $pna->extract_httpstat();
length($extract_httpstat) eq '155' ? ok(99) : nok(99);
md5_hex($extract_httpstat) eq 'c1bc3f7672aa91c3306b75f9c7c7cc9f' ? ok(100) : nok(100);
...
length($extract_ifconfig_a) eq '1227' ? ok(105) : nok(105);
md5_hex($extract_ifconfig_a) eq 'a7695636c076246ea1f4788b40a5e402' ? ok(106) : nok(106);
...
length($extract_ifstat_a) eq '9601' ? ok(111) : nok(111);
md5_hex($extract_ifstat_a) eq '22aeeedfde5a3cc2b3f132c46ba3fe71' ? ok(112) : nok(112);
...
length($extract_messages) eq '29524' ? ok(174) : nok(174);
md5_hex($extract_messages) eq 'ab7c0f8761bf96d526753ecab8138209' ? ok(175) : nok(175);
...
length($extract_netstat_s) eq '4067' ? ok(180) : nok(180);
md5_hex($extract_netstat_s) eq '16c2a1b551fc0777c9c15dfed42afb1b' ? ok(181) : nok(181);
...
length($extract_nfsstat_d) eq '7061' ? ok(186) : nok(186);
md5_hex($extract_nfsstat_d) eq '6b4c757c8333cf1bd2346f5bcc0fd33d' ? ok(187) : nok(187);
...
md5_hex($extract_nsswitch_conf) eq '7f10ab464a2b4cf37137da3c6d1b9300' ? ok(193) : nok(193);
substr($extract_nsswitch_conf,0,20) eq '===== NSSWITCH-CONF ' ? ok(194) : nok(194);

my $extract_options = $pna->extract_options();
length($extract_options) eq '15751' ? ok(195) : nok(195);
md5_hex($extract_options) eq '75a8398e04be36d081cfad73fc1ec14f' ? ok(196) : nok(196);
...
length($extract_qtree_status) eq '561' ? ok(204) : nok(204);
md5_hex($extract_qtree_status) eq 'c5f00806e980d324ba17e210932310b5' ? ok(205) : nok(205);
...
length($extract_rc) eq '419' ? ok(210) : nok(210);
md5_hex($extract_rc) eq '618b266cd70a567539e14970bbe2a30c' ? ok(211) : nok(211);
...
md5_hex($extract_resolv_conf) eq '6734a50150ab3ab3761eb5debd54dfb4' ? ok(214) : nok(214);
...
length($extract_service_usage) eq '469' ? ok(234) : nok(234);
md5_hex($extract_service_usage) eq '14a7f2dfa9a6a8269e303f45465f272e' ? ok(235) : nok(235);
...
length($extract_snap_list_n) eq '715' ? ok(261) : nok(261);
md5_hex($extract_snap_list_n) eq 'dc70401719d67663d941b2ff7e62eb6b' ? ok(262) : nok(262);
...
length($extract_snap_reserve) eq '572' ? ok(267) : nok(267);
md5_hex($extract_snap_reserve) eq '1bcb75544b79a52b48343911b4b0c430' ? ok(268) : nok(268);
...
length($extract_snap_sched) eq '219' ? ok(273) : nok(273);
md5_hex($extract_snap_sched) eq 'df7ee670ea9268fb1756cdb28a0a5530' ? ok(274) : nok(274);
substr($extract_snap_sched,0,20) eq '===== SNAP-SCHED ===' ? ok(275) : nok(275);

my $extract_snap_sched_a = $pna->extract_snap_sched_a();
length($extract_snap_sched_a) eq '49' ? ok(276) : nok(276);
md5_hex($extract_snap_sched_a) eq '028859c82e98d94adfc347cd83c9d1f6' ? ok(277) : nok(277);
substr($extract_snap_sched_a,0,20) eq '===== SNAP-SCHED-A =' ? ok(278) : nok(278);

my $extract_snap_status = $pna->extract_snap_status();
length($extract_snap_status) eq '1019' ? ok(279) : nok(279);
md5_hex($extract_snap_status) eq 'f8079547e14b26e6ff20a2e95d2873b0' ? ok(280) : nok(280);
substr($extract_snap_status,0,20) eq '===== SNAP-STATUS ==' ? ok(281) : nok(281);

my $extract_snap_status_a = $pna->extract_snap_status_a();
length($extract_snap_status_a) eq '63' ? ok(282) : nok(282);
md5_hex($extract_snap_status_a) eq '33cf56617142ca8d3483abfb2fccc66e' ? ok(283) : nok(283);
...
length($extract_software_licenses) eq '1295' ? ok(306) : nok(306);
md5_hex($extract_software_licenses) eq '1a2729d98ce0a623ff114f79fcba6d17' ? ok(307) : nok(307);
...
length($extract_storage) eq '15123' ? ok(312) : nok(312);
md5_hex($extract_storage) eq '3bebd49551481332181edb745ba1d0ee' ? ok(313) : nok(313);
substr($extract_storage,0,20) eq '===== STORAGE =====
' ? ok(314) : nok(314);

my $extract_sysconfig_a = $pna->extract_sysconfig_a();
length($extract_sysconfig_a) eq '8066' ? ok(315) : nok(315);
md5_hex($extract_sysconfig_a) eq '3fbae8b455e39f9005d151b950b9f34b' ? ok(316) : nok(316);
...
length($extract_sysconfig_d) eq '1021' ? ok(324) : nok(324);
md5_hex($extract_sysconfig_d) eq '080c41321bafc14c98ef14078eaeabf9' ? ok(325) : nok(325);
...
md5_hex($extract_sysconfig_hardware_ids) eq '821bb12c075f57372f1bdb77e047eaa3' ? ok(328) : nok(328);
...
length($extract_sysconfig_m) eq '2429' ? ok(330) : nok(330);
md5_hex($extract_sysconfig_m) eq '86aeae563abf6ad14197cbb3b1eb1712' ? ok(331) : nok(331);
substr($extract_sysconfig_m,0,20) eq '===== SYSCONFIG-M ==' ? ok(332) : nok(332);

my $extract_sysconfig_r = $pna->extract_sysconfig_r();
length($extract_sysconfig_r) eq '1887' ? ok(333) : nok(333);
md5_hex($extract_sysconfig_r) eq '3b23f1d126dfd8a525af3ec826453fa9' ? ok(334) : nok(334);
...
md5_hex($extract_system_serial_number) eq 'f6a600c21bdca6bc352b7a1278661dca' ? ok(337) : nok(337);
...
length($extract_usage) eq '2998' ? ok(342) : nok(342);
md5_hex($extract_usage) eq 'a793773698ca35f838f62dff0e940287' ? ok(343) : nok(343);
...
length($extract_vif_status) eq '1660' ? ok(354) : nok(354);
md5_hex($extract_vif_status) eq 'b6934ec9e19cf81410a17b31558d9a79' ? ok(355) : nok(355);
...
length($extract_vol_language) eq '411' ? ok(360) : nok(360);
md5_hex($extract_vol_language) eq '3a4197aa2b17126f554cea4d0d491bac' ? ok(361) : nok(361);
substr($extract_vol_language,0,20) eq '===== VOL-LANGUAGE =' ? ok(362) : nok(362);

my $extract_vol_status = $pna->extract_vol_status();
length($extract_vol_status) eq '13329' ? ok(363) : nok(363);
md5_hex($extract_vol_status) eq '66682eee53b3fdd1b49e7c226bfde0ea' ? ok(364) : nok(364);
...
length($extract_xheader) eq '618' ? ok(375) : nok(375);
md5_hex($extract_xheader) eq '8cf0caff3ec3147e9f5aaea5e1f81d79' ? ok(376) : nok(376);
